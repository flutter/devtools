...
/// flutter/packages/flutter_tools/lib/src/cache.dart. It supports the use of
/// [FlutterTestDriver].
  ...
  Future<void> updateAll() async {
    ...
    try {
      for (CachedArtifact artifact in _artifacts) {
        if (!artifact.isUpToDate()) await artifact.update();
      }
...
abstract class CachedArtifact {
  ...
  bool isUpToDate() {
    ...
    if (version != cache.getStampFor(name)) return false;
    return isUpToDateInner();
  }

  Future<void> update() async {
    if (location.existsSync()) location.deleteSync(recursive: true);
  ...
  Future<void> _downloadArchive(String message, Uri url, Directory location,
    ...
    return _withDownloadFile('${flattenNameSubdirs(url)}',
      ...
      if (!verifier(tempFile)) {
        final Status status =
            logger.startProgress(message, expectSlowOperation: true);
        try {
...
class FlutterEngine extends CachedArtifact {
  ...
  List<List<String>> _getBinaryDirs() {
    ...
    if (cache.includeAllPlatforms)
      ...
      binaryDirs..addAll(_linuxBinaryDirs)..addAll(_androidBinaryDirs);
      ...
      binaryDirs..addAll(_windowsBinaryDirs)..addAll(_androidBinaryDirs);
  ...
  bool isUpToDateInner() {
    ...
    for (String pkgName in _getPackageDirs()) {
      ...
      if (!fs.directory(pkgPath).existsSync()) return false;
    }
  ...
  Future<void> updateInner() async {
    ...
    for (List<String> toolsDir in _getBinaryDirs()) {
      ...
      final File frameworkZip =
          fs.file(fs.path.join(dir.path, 'Flutter.framework.zip'));
      if (frameworkZip.existsSync()) {
        final Directory framework =
            fs.directory(fs.path.join(dir.path, 'Flutter.framework'));
  ...
  Future<bool> areRemoteArtifactsAvailable(
      {String engineVersion, bool includeAllPlatforms = true}) async {
    ...
    Future<bool> checkForArtifacts(String engineVersion) async {
      ...
      final String url =
          '$_storageBaseUrl/flutter_infra/flutter/$engineVersion/';
  ...
  void _makeFilesExecutable(Directory dir) {
    for (FileSystemEntity entity in dir.listSync()) {
      if (entity is File) {
        ...
        if (name == 'flutter_tester') os.makeExecutable(entity);
      }
    }
  }
}
...
String _flattenNameNoSubdirs(String fileName) {
  ...
  for (int codeUnit in fileName.codeUnits) {
    replacedCodeUnits
        .addAll(_flattenNameSubstitutions[codeUnit] ?? <int>[codeUnit]);
  }

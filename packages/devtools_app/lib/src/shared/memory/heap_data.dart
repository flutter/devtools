...
class HeapData {
  ...
  Future<void> _calculate({
    ...
    DebugTimer.snapshot?.printTime('calculation started');

    List<int>? retainers;

    if (calculateRetainingPaths || calculateRetainedSizes) {
      final weakClasses = _WeakClasses(graph);
      DebugTimer.snapshot?.printTime('weak classes detected');
      ...
      DebugTimer.snapshot?.printTime('shortest paths found');
    ...
    DebugTimer.snapshot?.printTime('calculation done');

...
class VmServiceWrapper implements VmService {
  ...
  static List<int> getTheStack(CpuProfileData data, String id) {
    final stack = <int>[];
    final parentMap = <String, String>{};
    String? current = id;

    final frame = data.stackFrames.forEach((id, frame) {
      if (id != frame.id) {
        throw Exception('THE FRAME ID WAS NOT EQ to key id');
      }
    ...
    while (current != null && current != 'cpuProfileRoot') {
      final theIndex =
          data.stackFrames.entries.map((e) => e.key).toList().indexOf(current);
      if (theIndex == null) {
        throw new Exception('YOUR INDEX ASSUMPTION WAS WRONG');
      }
      stack.add(theIndex);
  ...
  static CpuSamples generateCpuSamples(
    CpuProfileData profileData,
  ) {
    final functionData = <Map<String, String>>[];
    final sampleData = <Map<String, String>>[];
    final functions = <ProfileFunction>[];
    final samples = <CpuSample>[];
    // iterate through stack frames
    profileData.stackFrames.forEach((key, data) {
      functions.add(
        ProfileFunction(
          kind: 'Native',
          inclusiveTicks: data.inclusiveSampleCount,
          exclusiveTicks: data.exclusiveSampleCount,
          resolvedUrl: data.rawUrl,
          function: FuncRef(
            name: data.name,
            owner: null,
            isStatic: false,
    ...
    for (var sample in profileData.cpuSamples) {
      final id = int.parse(sample.leafId.split('-')[1]);
      final stack = getTheStack(profileData, sample.leafId);
      samples.add(
        CpuSample(
          tid: sample.threadId,
    ...
    final metadata = profileData.profileMetaData;
    return CpuSamples(
      samplePeriod: metadata.samplePeriod,
      maxStackDepth: metadata.stackDepth,
      sampleCount: metadata.sampleCount,
      timeSpan: -1,
      timeOriginMicros: metadata.time?.start?.inMicroseconds,
      timeExtentMicros: metadata.time!.end!.inMicroseconds -
          metadata.time!.start!.inMicroseconds,
      pid: profileData.cpuSamples[0].processId,
      functions: functions,
      samples: samples,
    );
  }

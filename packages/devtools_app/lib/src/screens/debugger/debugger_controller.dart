...
class DebuggerController extends DisposableController
  ...
  void onServiceShutdown() {
    ...
    _lastEvent = null;
    _currentScriptRef!.value = null as ScriptRef;
    _scriptLocation!.value = null as ScriptLocation;
    _stackFramesWithLocation.value = [];
    _selectedStackFrame.value = null as StackFrameAndSourcePosition;
    _variables.value = [];
    _breakpoints.value = [];
    _breakpointsWithLocation.value = [];
    _selectedBreakpoint.value = null as BreakpointAndSourcePosition;
  ...
  final ValueNotifier<ScriptRef>? _currentScriptRef = ValueNotifier<ScriptRef>(null as ScriptRef);

  ValueListenable<ScriptRef> get currentScriptRef => _currentScriptRef!;

  @visibleForTesting
  final ValueNotifier<ParsedScript>? parsedScript = ValueNotifier<ParsedScript>(null as ParsedScript);
  ...
  final ValueNotifier<ScriptLocation>? _scriptLocation = ValueNotifier<ScriptLocation>(null as ScriptLocation);
  ...
  Future<Class?> classFor(ClassRef classRef) async {
    try {
      if(_clazzCache[classRef] != null) {
        return _clazzCache[classRef];
      }

      await getObject(classRef);
    } catch (_) {}
    return null;
  }
  ...
  final _selectedStackFrame = ValueNotifier<StackFrameAndSourcePosition>(null as StackFrameAndSourcePosition);
  ...
  Frame get frameForEval {
    ...
    if(_selectedStackFrame.value.frame != null) {  //dep
      return _selectedStackFrame.value.frame!;
    }

    return _stackFramesWithLocation.value.safeFirst!.frame!;
  }
  ...
  final _selectedBreakpoint = ValueNotifier<BreakpointAndSourcePosition>(null as BreakpointAndSourcePosition);
  ...
  Future<String?> retrieveFullStringValue(
    ...
    final String Function(String?)? onUnavailableLocal = onUnavailable as String Function(String? p1)?;
    return serviceManager.service!.retrieveFullStringValue(
      isolateRef!.id!,
      stringRef,
      onUnavailable: onUnavailableLocal,
    );
  }
  ...
  void _updateAfterIsolateReload(Event reloadEvent) async {
    ...
    if (removedScripts.contains(currentScriptRef.value)) {
      ...
      final newScriptRef = addedScripts
          .firstWhereOrNull((script) => script.uri == uri);

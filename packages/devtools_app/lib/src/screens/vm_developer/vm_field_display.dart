...
class FieldInfoWidget extends StatelessWidget implements PreferredSizeWidget {
  ...
  String _observedTypes(FieldObject field) {
    ...
    if (field.guardClassKind == guardClassSingle) {
      type = field.guardClass!.name ?? '<Observed Type Name>';
    } else if (field.guardClassKind == guardClassUnknown) {
      type = 'none';
    } else if (field.guardClassKind == guardClassDynamic) {
      type = guardClassDynamic;
    } else {

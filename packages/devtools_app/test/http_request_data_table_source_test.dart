...
void main() {
  group('HttpRequestDataTableSource', () {
    ...
    setUpAll(() async {
      ...
      httpTestData = jsonDecode(await File(testDataPath).readAsString());
    ...
    void dataTableListenerScope(Function listener, Function callback) {
    ...
    test('verify utilities', () {
      ...
      expect(dataTable.formatRequestTime(DateTime(2020, 1, 16, 13, 0, 0)),
          '13:00:00 1/16/2020');
    });
    ...
    test('select row', () {
      ...
      dataTableListenerScope(selectListener, () => row.onSelectChanged(true));
      ...
      dataTableListenerScope(
          deselectListener, () => dataTable.clearSelection());

      // and with onSelectChanged(false)
      row.onSelectChanged(true);
      dataTableListenerScope(
          deselectListener, () => row.onSelectChanged(false));
    });
    ...
    test('sorting', () {
      ...
      void verifySorting(Function(HttpRequestData) getField) {
        bool ascending = true;
        dataTableListenerScope(() => verifyOrder(getField, ascending),
            () => dataTable.sort(getField, ascending));
      ...
      verifySorting((data) => data.name);
      verifySorting((data) => data.method);
      verifySorting((data) => data.status);
      verifySorting((data) => data.duration);
      verifySorting((data) => data.requestTime);
    });
  });
}

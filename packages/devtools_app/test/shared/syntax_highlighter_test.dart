...
const functionSpans = ['foo()', '_foo()', 'foo(bar)'];

const numericSpans = ['1', '1.1', '0xFF', '0xff', '1.3e5', '1.3E5'];
...
void main() {
  ...
  void spanTester(
    ...
    expect(
      span.style,
      TextStyle(color: expectedColor(Theme.of(context).colorScheme)),
    );
  }

  void runTestsWithTheme({required bool useDarkTheme}) {
    group('Syntax Highlighting (${useDarkTheme ? 'Dark' : 'Light'} Theme)', () {
      ...
      testWidgetsWithContext('hello world smoke', (WidgetTester tester) async {
        ...
        await tester.pumpWidget(
          buildSyntaxHighlightingTestContext((context) {
            ...
            spanTester(context, children[1] as TextSpan, '<', defaultTextColor);
            ...
            spanTester(context, children[3] as TextSpan, '>', defaultTextColor);

            spanTester(context, children[4] as TextSpan, ' ', defaultTextColor);
      ...
      testWidgetsWithContext('multiline documentation', (
        WidgetTester tester,
      ) async {
        final highlighter = SyntaxHighlighter.withGrammar(
          grammar: grammar,
      ...
      testWidgetsWithContext('documentation code reference', (
        WidgetTester tester,
      ) async {
        final highlighter = SyntaxHighlighter.withGrammar(
          grammar: grammar,
      ...
      testWidgetsWithContext('variable reference in string', (
        WidgetTester tester,
      ) async {
        final highlighter = SyntaxHighlighter.withGrammar(
          grammar: grammar,
      ...
      void testSingleSpan(
        ...
        testWidgetsWithContext("$name '$spanText'", (
          WidgetTester tester,
        ) async {
          final highlighter = SyntaxHighlighter.withGrammar(
            grammar: grammar,
          ...
          await tester.pumpWidget(
            buildSyntaxHighlightingTestContext((context) {
              ...
              expect(
                ...
                TextStyle(color: colorCallback(Theme.of(context).colorScheme)),
              );
            }),
          );
        });
      ...
      group('single span highlighting:', () {
        ...
        for (final spanText in controlFlowSpans) {
          testSingleSpan('control flow', spanText, controlFlowSyntaxColor);
        }

        for (final spanText in declarationSpans) {
          testSingleSpan('declaration', spanText, declarationSyntaxColor);
        }

        for (final spanText in functionSpans) {
          testSingleSpan('function', spanText, functionSyntaxColor);
        }

        for (final spanText in numericSpans) {
          testSingleSpan('numeric', spanText, numericConstantSyntaxColor);
        }
      });
    });
  }

...
Events are sent as objects with a string `event` field and an object `params` field.

#### server.started Event

This event is sent when the server starts up and has bound to a port. `params` contains the following fields:
...
`launchDevTools` is registered as a VM service and takes the following `params`:
